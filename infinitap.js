window.onload = function(){
    ...
    let randomQuote, randomEmoji, levelNum = 1, clickNum = 0; 
    let getScore, scoreArr =[];
    
    const $ = (selector) => document.querySelectorAll(selector);
    const random = (item) => Math.floor(Math.random()*item); 
  
    const hideAllPages = () => allPages.forEach(
        (page)=> { 
            (page.classList.contains("show"))? page.classList.replace("show","hide"): page.classList.add("hide");
        });
    ...
    function showPage(event){        
        ...
        (homePage.classList.contains("show"))? homePage.classList.replace("show","hide"): homePage.classList.add("hide");
    }
    
    function goHome(event){
        hideAllPages();
        (homePage.classList.contains("hide"))? homePage.classList.replace("hide","show"): homePage.classList.add("show");   
    }
    ...
    function ScoreBoard(scores){
        // empty score check
        let isEmpty = [null, undefined,0].includes(scores.Clicks);
        // duplicateCheck
            let jsonScore = JSON.stringify(scores); 
            let jsonArr =scoreArr.map((scores)=> JSON.stringify(scores)); 
            if (!isEmpty && (!jsonArr.includes(jsonScore))) scoreArr.push(scores);
    ...
    // init
    hideAllPages();
    $(".info").forEach((elem)=> elem.insertAdjacentHTML("beforeend",backButtonHtml));
    [...homeButtons].forEach((elem)=>elem.addEventListener("click",showPage));
    [navButton,...backButton].forEach((elem)=> elem.addEventListener("click",goHome) );
    navButton.addEventListener("click", resetGame);
